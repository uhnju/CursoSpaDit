...
import { axiosAPI, Utilidades, Alert, LoadingOverlay, DataGrid } from "@dit/spad-rdit_library";
import { validaNif } from "./util";
import { EmpleadosTypes } from "./EmpleadosTypes"
interface Datos {
  headCells: string[];
  rows: { value: string, dataPulsable?: string }[][];
}
type DataGridDetalleRowProps = {
  value: string;
  dataPulsable?: string;
  tooltip?: string;
  linkValue?: string
...
type DataGridRowProps = {
  [key: string]: DataGridDetalleRowProps;
}

const ConsultaEmpleadosConDataGrid = () => {
  const configDataGrid = {
    id: "ejemploDataGrid",
    tableTitle: "ejemploDataGrid"
  }
  const datosInicialesDataGrid = {
    headCells: ["Nif", "Grupo", "Nivel", "Nombre"],
    rows: []
  }
  ...
  const [datosDataGrid, setDatosDataGrid] = useState<Datos>(datosInicialesDataGrid);
  ...
  const buscarEmpleados = async () => {
    ...
    try {
      ...
      let arrayDataGrid: { value: string, dataPulsable?: string }[][] = [];
      data.respuesta.empleados.forEach(empleado => {
        arrayDataGrid.push([
          { value: empleado.nif, dataPulsable: empleado.nif },
          { value: empleado.grupo },
          { value: empleado.nivel.toString() },
          { value: empleado.nombre },
        ]);
      })
      setDatosDataGrid({ ...datosDataGrid, rows: arrayDataGrid });
    } catch (error) {
  ...
  const handleCellClick = (row: DataGridRowProps) => {
    navigate(`../DetalleEmpleado?nif=${row["Nif"].dataPulsable}`)
  }
  ...
  return (
    <LoadingOverlay loading={loading} >
      ...
      <div className="container">
        ...
        <DataGrid config={configDataGrid} data={datosDataGrid} onClickCell={(row) => handleCellClick(row)} />
      </div>
    </LoadingOverlay>
  );
};
export default ConsultaEmpleadosConDataGrid;
